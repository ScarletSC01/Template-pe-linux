            ...
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'JIRA_TOKEN', usernameVariable: 'JIRA_USER', passwordVariable: 'JIRA_API_TOKEN')]) {
                        ...
                        env.ESTADO_TICKET = sh(script: "bash -c 'curl -s -u \"$JIRA_USER:$JIRA_API_TOKEN\" -X GET \"${JIRA_API_URL}${params.TICKET_JIRA}\" -H \"Accept: application/json\" | jq -r \".fields.status.name // \\\"Desconocido\\\"\"'", returnStdout: true).trim()
                        ...
                        if (transitionId) {
                            sh(script: "curl -s -u '${JIRA_USER}:${JIRA_API_TOKEN}' -X POST '${JIRA_API_URL}${params.TICKET_JIRA}/transitions' -H 'Content-Type: application/json' --data '${groovy.json.JsonOutput.toJson([transition: [id: transitionId]])}'")
                            env.TRANSICION_REALIZADA = 'true'
                        }

                        def comentario = groovy.json.JsonOutput.toJson([body: "Validación completada. El ticket ${params.TICKET_JIRA} se encontraba en estado ${env.ESTADO_TICKET}."])
                        sh(script: "curl -s -u '${JIRA_USER}:${JIRA_API_TOKEN}' -X POST '${JIRA_API_URL}${params.TICKET_JIRA}/comment' -H 'Content-Type: application/json' --data '${comentario}'")
                   }
                }
            }
        }
        ...
        stage('Notificar a Teams') {
            steps {
                script {
                    ...
                    sh(script: "curl -X POST -H 'Content-Type: application/json' --data-raw '${payload}' ${TEAMS_WEBHOOK}")
                }
            }
        }
    }
